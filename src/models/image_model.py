...
class IntraNoAR(CompressionModel):
    ...
    def compress(self, x, q_scale):
        ...
        y = self.enc(x) # Contextual Encode，对输入图像进行编码，得到特征y
        y = y / curr_q # Multi - Granularity Quant，得到y2t
        z = self.hyper_enc(y) # 使用超编码器，对特征y进行编码，得到更紧凑的表示z，作为熵模型的输入
        z_hat = torch.round(z)

        params = self.hyper_dec(z_hat) #fig.2.的HPD，对z解码，得到参数
        q_step, scales, means = self.y_prior_fusion(params).chunk(3, 1) # Prior Fusion + Param Estimation

        #! 双重先验，Quant，Code1，Code2
        y_q_w_0, y_q_w_1, scales_w_0, scales_w_1, y_hat = self.compress_dual_prior(
            y, means, scales, q_step, self.y_spatial_prior)
        y_hat = y_hat * curr_q

        #todo 熵编码，可能包含了算术编码器的部分？
        ...
        bit_stream = self.entropy_coder.flush_encoder() # 得到编码后的比特流
